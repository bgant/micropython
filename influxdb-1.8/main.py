...
if 'Milone_eTape.py' in listdir():
    ...
    def read_sensor():
        return "%s,device=%s value=%d" % (measurement, client_id, pin.value())
...
print('Data Pin:        %s' % SENSOR_PIN)
print('Data Interval:   %s seconds' % sleep_interval)
...
def main(trigger=None):  # trigger variable contains settings of pin or timer that caused interrupt (if used)
    ...
    if '204' in str(response.status_code):  # HTTP Status 204 (No Content) indicates server fulfilled request
        print(f'DB: {database} \t Measurement: {data} \t Status: {response.status_code} Success')
        if 'TinyPICO' in uname().machine:
            led.blink(0,255,0,ms=2500,i=1) # Green
        wdt.feed()  # Reset Hardware Watchdog Timer
    else:
        print(f'DB: {database} \t Measurement: {data} \t Status: {response.status_code} Failed')
        if 'TinyPICO' in uname().machine:
...
if __name__ == '__main__':
    ...
    timer = Timer(0)
    timer.init(period=sleep_interval*1000, mode=Timer.PERIODIC, callback=main)
